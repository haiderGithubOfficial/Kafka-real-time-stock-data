...
cron.schedule('1 10-16 * * 1-5', async function () {
    try {
        console.log('Cron Job Executed at:', moment().format('M_D_YYYY-H_m'));
        const targetDate = moment().subtract(1, 'hours').startOf('hour').format('YYYY-MM-DD HH:00:00');
        ...
        _.mapValues(groupedData, (entries) => {
            candles.push({
                ...
                candleTime: moment(_.head(entries).feed_time).minutes(0).seconds(0).milliseconds(0).format('YYYY-MM-DD HH:mm:ss')
            })
        });
        ...
        try {
            ...
            console.log('1 hour Candles created successfully');
        } catch (error) {
            console.log(error);
        }
